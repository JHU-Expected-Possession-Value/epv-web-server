"""
Plot pass probabilities to all teammates for a given match/time.

Usage:
  python3 diagnostics/plot_pass_probabilities.py --match-id 1039803 --time 12:34
"""
...
def _pass_probability(
    ...
    pc_dest = 0.5
    pc_path_min = 0.5
    if calc.pc_runner and frame > 0:
        try:
            pc_origin = calc.pc_runner.pc_at_point(frame, x_origin, y_origin)
            pc_dest = calc.pc_runner.pc_at_point(frame, x_dest, y_dest)
            pc_path_min = calc._pitch_control_path_min(
                x_origin, y_origin, x_dest, y_dest, frame
            )
        except Exception:
            pass

    defenders_near_origin = calc._count_defenders_within_distance(
        x_origin, y_origin, frame_data, team_id, distance=3.0
    )
    defenders_near_dest = calc._count_defenders_within_distance(
        x_dest, y_dest, frame_data, team_id, distance=3.0
    )
    defenders_in_lane = calc._count_defenders_in_passing_lane(
        x_origin, y_origin, x_dest, y_dest, frame_data, team_id
    )
    min_defender_dist_to_lane = calc._min_defender_dist_to_passing_lane(
        x_origin, y_origin, x_dest, y_dest, frame_data, team_id
    )

    player_passing_skill = calc.passing_skills.get(player_id, 0.0)

    all_teams = list(calc.current_team_roster.keys())
    defending_team_id = [t for t in all_teams if t != team_id][0] if len(all_teams) > 1 else team_id
    last_defensive_line_x, last_defensive_line_height = calc._calculate_defensive_line(
    ...
    if calc.passing_feature_cols:
        features = np.array([[feature_dict[col] for col in calc.passing_feature_cols]])
    else:
        features = pd.DataFrame([feature_dict]).values

    try:
        return float(calc.passing_model.predict_proba(features)[0, 1])
    except Exception as exc:
        print(f"  WARN: Passing prediction error: {exc}")
        return 0.5


def plot_pass_probabilities(
    match_id: str,
    time_str: str,
    ...
    teammates = [
        p for p in positions["own"]
        if p.get("pid") is not None and p.get("pid") != pid
    ]
    if not teammates:
        raise RuntimeError("No teammates detected in this frame")

    pass_rows: List[Dict[str, float | int]] = []
    for teammate in teammates:
        dest_x, dest_y = teammate["x"], teammate["y"]
        p_success = _pass_probability(
            calc, x, y, dest_x, dest_y, frame, frame_data, pid, tid
        )
        pass_rows.append(
            {
                "player_id": teammate.get("pid"),
                "x": float(dest_x),
                "y": float(dest_y),
                "p_success": p_success,
            }
    ...
    pass_rows = sorted(pass_rows, key=lambda r: r["p_success"], reverse=True)
    ...
    for row in pass_rows:
        ...
        ax.annotate(
            ...
            xy=(px, py),
            xytext=(x, y),
            arrowprops=dict(arrowstyle="->", color="tab:blue", lw=1.5, alpha=0.35 + 0.55 * prob),
            zorder=2,
        )
        ax.text(
            px + 0.6,
    ...
    ax.set_title(
        f"Pass probabilities @ {action_seconds/60:.2f} min (closest to {time_str})"
    )
    ...
    table_rows = [
        [str(row["player_id"]) if row["player_id"] is not None else "unknown", f"{row['p_success']:.3f}"]
        for row in pass_rows
    ]
    table = ax.table(
        cellText=table_rows,
        colLabels=["Teammate", "P(pass)"],
        colLoc="center",
        cellLoc="center",
        bbox=[0.02, 0.10, 0.22, 0.62],
    )
    table.auto_set_font_size(False)
    table.set_fontsize(7)

    safe_time = time_str.replace(":", "-").replace(".", "p")
    out_path = results_dir / f"pass_probs_{match_id}_{safe_time}.png"
    fig.tight_layout()
...
def main():
    ...
    out_path = plot_pass_probabilities(args.match_id, args.time)
